...
TCGA_firehose_Legacy_clinical_data = read.delim("~/Codes_for_manuscript_outputs/InPut_datasets/TCGA_firehose_Legacy_clinical_data_31May2022_10amgmt.tsv")
...
TCGA_Pancancer_Atlas_clinical_data = read.delim("~/Codes_for_manuscript_outputs/InPut_datasets/TCGA_Pancancer_Atlas_clinical_data_31May2022_10amgmt.tsv")
#---------------------------------------------------------------------------------------------------------------------
...
```{r Cancer vs pre-ART and post-ART study}
#ART comparison 90' and 95'
#Load dataframe from literature
CC_ART_trends = fread("~/Codes_for_manuscript_outputs/InPut_datasets/CC_data_pre_post_ART.txt")

CC_ART_trends$era = factor(CC_ART_trends$era,
                                  levels = c('Pre-ART', 'Post-ART'), ordered = TRUE)
CC_ART_trends$cancer_type = factor(CC_ART_trends$cancer_type,
                                  levels = c('KS', 'NHL', 'CC'))

#plot
p1=ggplot(CC_ART_trends, aes(x = era, y = cases, fill = era, label = cases)) +
  geom_bar(position = "stack", stat = "identity", width = 0.7,colour="black") +
  scale_fill_manual(values = c("darkorange2","darkturquoise"))+
  geom_text(aes(label = cases),position = position_dodge(width = 0.7), vjust = 1, color = "black", size = 2.5) +
...
pdf(file = "Figure 1B-ART_CC_trend.pdf", width = 3.7, height=2.3, useDingbats = FALSE)
print(p1)
dev.off()
